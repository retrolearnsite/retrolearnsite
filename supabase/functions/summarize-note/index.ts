...
serve(async (req) => {
  ...
  try {
    ...
    if (!aiData.candidates || aiData.candidates.length === 0) {
      ...
      // Fallback to Lovable AI Gateway (Gemini) if Google response has no content
      try {
        ...
        const aiResp = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: 'system', content: 'You are an expert note summarizer. Respond with exactly 3 concise lines. No preamble or conclusion.' },
              { role: 'user', content: `Title: ${note.title || 'Untitled'}\n\nNote:\n${note.original_content}\n\nTask: Summarize in 3 concise lines.` },
            ],
          }),
        });
        ...
        if (aiResp.ok) {
          ...
          if (content) {
            ...
            return new Response(
              JSON.stringify({ success: true, summary: content }),
              { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
            );
          }
        } else {
          const errTxt = await aiResp.text();
          console.error('Lovable AI gateway error (no candidates path):', aiResp.status, errTxt);
          await logApiUsage(
            supabaseClient,
            'summarize-note',
            'lovable_ai',
        ...
        console.error('Lovable AI fallback failed (no candidates path):', fbErr);
      }
    ...
    if (!summary) {
      console.error('No summary text in response:', JSON.stringify(aiData, null, 2));
      await logApiUsage(supabaseClient, 'summarize-note', 'gemini', usedModel, false, 'error', 'No summary text', responseTime);

      // Final fallback to Lovable AI Gateway (Gemini) if Google returns empty content
      try {
        const LOVABLE_API_KEY = Deno.env.get('LOVABLE_API_KEY');
        if (!LOVABLE_API_KEY) throw new Error('LOVABLE_API_KEY not configured');
        ...
        if (aiResp.ok) {
          ...
          const errTxt = await aiResp.text();
          console.error('Lovable AI gateway error (no summary path):', aiResp.status, errTxt);
          await logApiUsage(
            supabaseClient,
            'summarize-note',
            'lovable_ai',
        ...
        console.error('Lovable AI fallback failed (no summary path):', fbErr);
      }
