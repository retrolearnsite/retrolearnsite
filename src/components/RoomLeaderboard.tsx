...
export default function RoomLeaderboard({
  ...
  const fetchLeaderboard = async () => {
    ...
    const {
      data: xpData,
      error: xpError
    } = await supabase.from('room_xp_activity').select('user_id, xp_earned').eq('room_id', roomId);
    if (xpError) {
    ...
    }).sort((a, b) => b.total_xp - a.total_xp).map((entry, index) => ({
      ...entry,
      rank: index + 1
    })).slice(0, 10);
  ...
  if (loading) {
    return <Card className="border-2 border-accent/30 bg-card/90 backdrop-blur-sm">
        <CardHeader>
  ...
  return <Card className="border-2 border-accent/30 bg-card/90 backdrop-blur-sm">
      <CardHeader>
        <CardTitle className="font-retro text-xl glow-text">Leaderboard<Trophy className="w-5 h-5 inline mr-2" />
          LEADERBOARD
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-2">
          {entries.map(entry => <div key={entry.user_id} className={`
                flex items-center gap-3 p-3 rounded-lg border transition-all
                ${entry.rank <= 3 ? 'border-accent/50 bg-accent/10' : 'border-border/30 bg-muted/20'}
              `}>
              ...
              <Badge variant={entry.rank <= 3 ? "default" : "secondary"} className="font-retro text-xs">
                #{entry.rank}
              </Badge>
            </div>)}
        </div>
